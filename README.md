...
> 也可以点顶栏的数据源按钮，授权选本地文件夹直接读 —— 本地改完切回浏览器
> 就生效，不用重新部署。**可以挂多个目录，它们在顶栏切换、各看各的（不合并）。**
> **笔记本身不需要联网，也不需要提交。**
...
│   │   └── fsaccess.js             ← File System Access API 封装（多个目录句柄的存取）
│   ├── composables/
│   │   └── useLibrary.js           ← 模块级单例：数据源列表 / 切换 / 索引 / 自动同步
│   ├── components/
│   │   ├── views/                  ← 四个视图
│   │   ├── NoteDrawer.vue          ← 详情抽屉
│   │   ├── GlobalSearch.vue        ← 全局搜索（/ 聚焦、↑↓、↵）
│   │   └── TopBar.vue              ← 顶栏 + 数据源切换器
...
点顶栏右上角的数据源按钮，选一个文件夹，浏览器会要一次授权。
之后本地改完文件、切回浏览器就生效（`focus` 事件 + 5s 轮询双保险），不用重新部署。

### 可以挂多个目录，但它们是**彼此独立**的知识库

挂第二个目录：点顶栏的数据源按钮 →「＋ 连接本地目录」。**多个目录不会合并成一个库**，
而是各看各的，用同一个按钮切换：
...
**为什么不合并**：笔记的 id 就是**相对笔记根的路径**。两个仓库都有
`notes/design.md` 的话，合并会让它们撞成同一个 id —— 列表里只剩一篇，
正文里的交叉引用还会串到另一个仓库去。加个目录前缀能绕开，
但那样「承重墙」「避坑智库」这些**按项目维度**的统计也会混在一起，语义就糊了。

所以规则很简单：**一个挂载点 = 一个知识库**。

- 想分开看两个仓库 → 分别挂载它们
- 想当一个整体看 → 挂它们的共同父目录（会递归扫所有子目录）

每个数据源各自缓存，来回切换是瞬时的。热同步只作用于**当前正在看的**那个目录，
后台目录不会反复读盘。

### 三个会「静默漏文件」的地方

这三条都**不报错、不提示**，只是文件不见了：

| 情况 | 原因 |
| --- | --- |
| 笔记放在 `public/` `static/` `assets/` `dist/` 等目录下 | 这些目录在跳过清单里（原意是跳过构建产物，但内容目录也可能叫这个名） |
| 一个**只有 README 的仓库**，一篇都扫不到 | `README.md` / `CHANGELOG.md` / `LICENSE.md` 被当成「仓库说明」排除掉了 |
| `.git/` 和隐藏目录里的内容 | 自动跳过（这条是对的） |

所以某个目录挂上去是 0 篇时，先看它是不是只有 README。
...
- 目录句柄存在 **IndexedDB**（不是 localStorage —— 句柄不能被 JSON 序列化）。
  ...
  恢复时如果某个目录权限过期了，菜单里那一项会显示「需要授权」，点它才弹授权。
- 移除目录点菜单里那一项右边的 `×`，会同时清掉 IndexedDB 里的句柄。
  移除**当前正在看的**目录会自动退回内置数据。
- **本地目录里的内容不会上传**：整个站点是纯静态的，没有后端，
  运行时唯一的网络请求是拉内置的 `notes.json`。
  所以私有仓库的内容可以放心挂 —— 但反过来，**线上站点也永远读不到你本地挂载的目录**，
  它只存在于你这个浏览器里。要让它出现在线上，只能把内容提交进仓库，那也就是公开了。
...
- **多数据源各看各的、不合并，所以全局搜索只搜当前那一个库**。
  要跨目录找东西得先切过去。（为什么不合并见上面「连接本地目录」一节。）
